...
function initDatabase() {
  ...
  // Personal Installments - داواکاری قیستی کەسی
  db.exec(`
  ...
  // Migration: Add buildingExpenses column to monthly_expenses table if it doesn't exist
  try {
    const monthlyExpensesInfo = db.prepare("PRAGMA table_info(monthly_expenses)").all();
    const monthlyExpensesColumns = monthlyExpensesInfo.map(col => col.name);

    if (!monthlyExpensesColumns.includes('buildingExpenses')) {
      db.exec('ALTER TABLE monthly_expenses ADD COLUMN buildingExpenses REAL DEFAULT 0');
      console.log('✅ Added buildingExpenses column to monthly_expenses table');
    }
  } catch (error) {
    console.error('Migration error for monthly_expenses table:', error);
  }
