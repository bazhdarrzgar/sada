...
export function PrintButton({ 
  ...
  const generatePrintablePage = () => {
    if (!data || data.length === 0) {
      alert('هیچ داتایەک بۆ چاپکردن نییە')
      return
    }
    ...
    try {
      ...
      if (printWindow) {
        ...
        alert('پەنجەرەی چاپکردن نەکرایەوە. تکایە popup blocker چالاک نەکرێت.')
      }
      
    } catch (error) {
      console.error('Print generation error:', error)
      alert('خەطا لە ئامادەکردنی چاپکردن')
    } finally {
      setIsLoading(false)
    }
  }
