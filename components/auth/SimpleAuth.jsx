      ...
      try {
        ...
        if (typeof window !== 'undefined') {
          ...
          if (authToken === 'authenticated_berdoz') {
            console.log('SimpleAuth: User is authenticated')
            setIsAuthenticated(true)
          } else {
            console.log('SimpleAuth: User is not authenticated')
            // For testing purposes, auto-authenticate
            console.log('SimpleAuth: Auto-authenticating for testing')
            localStorage.setItem('auth_token', 'authenticated_berdoz')
            localStorage.setItem('username', 'berdoz')
            localStorage.setItem('user', JSON.stringify({username: 'berdoz', name: 'Berdoz Management'}))
            setIsAuthenticated(true)
          }
        }
      } catch (error) {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (username.trim() === 'berdoz' && password === 'berdoz@code') {
        ...
        setError('Invalid username or password. Use: berdoz / berdoz@code')
      }
    } catch (error) {
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    placeholder="Enter password (berdoz@code)"
                    value={password}
            ...
            <div className="mt-6 pt-4 border-t border-gray-200">
              <div className="text-center">
                ...
                <p className="text-xs text-gray-400 mt-1">
                  Demo credentials: berdoz / berdoz@code
                </p>
              </div>
            </div>
          </CardContent>
