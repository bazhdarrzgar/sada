          ...
          <Card key={staff.id} className="p-4 theme-card glow-button hover:shadow-xl transition-all duration-300 border-2 hover:border-blue-300 dark:hover:border-blue-500 group">
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div><span className="font-semibold">مۆبایل:</span> {staff.mobile}</div>
                <div><span className="font-semibold">بەش:</span> {staff.department}</div>
                <div><span className="font-semibold">ڕەگەز:</span> {staff.gender}</div>
                <div><span className="font-semibold">تەمەن:</span> {staff.age}</div>
                <div><span className="font-semibold">جۆری خوێن:</span> {staff.bloodType}</div>
                <div><span className="font-semibold">گرێبەست:</span> {staff.contract}</div>
                <div><span className="font-semibold">ئامادەبوون:</span> {staff.attendance}</div>
              </div>
              <div className="text-sm">
                <span className="font-semibold">ناونیشان:</span> {staff.address}
              </div>
              {staff.notes && (
                <div className="text-sm text-gray-600 dark:text-gray-400">
                  <span className="font-semibold">تێبینی:</span> {staff.notes}
                </div>
              )}
    ...
    <PageLayout title="Staff Records" titleKu="تۆمارەکانی ستاف">
      <div className="flex flex-col sm:flex-row gap-4 items-center justify-between">
        <div className="relative flex-1">
          ...
          <Input
            placeholder="گەڕانی فازی لە هەموو ستوونەکانی ستافدا... / Fuzzy search across all staff columns..."
            value={searchTerm}
      ...
      <EditModal
        ...
        titleKu="دەستکاریکردنی ئەندامی ستاف"
      />
      ...
      {filteredData.length === 0 && (
        <Card>
          <div className="p-8 text-center">
            ...
            <h3 className="text-lg font-semibold mb-2">هیچ ستافێک نەدۆزرایەوە</h3>
            <p className="text-gray-600 dark:text-gray-400">
              {searchTerm ? 'هیچ ئەنجامێک بۆ گەڕانەکەت نەدۆزرایەوە' : 'تا ئێستا هیچ ستافێک زیاد نەکراوە'}
            </p>
          </div>
        </Card>
      )}
