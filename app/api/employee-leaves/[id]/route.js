...
export async function PUT(request, { params }) {
  try {
    ...
    const result = await db.collection('employee_leaves').updateOne(
      { id },
      { $set: updateData }
    )
    ...
    const updatedRecord = await db.collection('employee_leaves').findOne({ id })
...
export async function DELETE(request, { params }) {
  try {
    ...
    const result = await db.collection('employee_leaves').deleteOne({ id })
