  ...
  function ActivitiesTableView({ data }) {
    ...
    const columns = [
      ...
      },
      {
        ...
        editable: false,
        render: (value, row) => {
          if (!value || value.length === 0) return <span className="text-gray-400">هیچ وێنەیەک نییە</span>
          
          return (
            <div className="flex items-center justify-center gap-2">
              {/* Show first few images as thumbnails */}
              <div className="flex -space-x-1 overflow-hidden">
                {value.slice(0, 3).map((image, imgIdx) => (
                  <img
                    key={imgIdx}
                    src={image.url}
                    alt={`Activity ${imgIdx + 1}`}
              ...
              >
                ...
                <span className="text-sm font-medium text-blue-600">{value.length}</span>
              </div>
            </div>
          )
        }
              ...
              <div className="space-y-4">
                ...
                <div className="flex justify-end gap-2">
                  ...
                  <Button onClick={() => saveEntry(newEntry)}>
                    {t('activities.buttons.save', language)}
                  </Button>
                </div>
              </div>
            </DialogContent>
