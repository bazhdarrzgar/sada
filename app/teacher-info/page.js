...
export default function TeacherInfoPage() {
  ...
  const fuse = useMemo(() => {
    const options = {
      keys: [
        { name: 'politicalName', weight: 0.25 }, // Main name field
        { name: 'specialty', weight: 0.2 }, // Important field
        { name: 'subject', weight: 0.15 }, // What they teach
        { name: 'program', weight: 0.1 }, // Education level
        { name: 'notes', weight: 0.1 }, // Additional info
        { name: 'bloodType', weight: 0.05 }, // Health info
        { name: 'totalHours', weight: 0.05 }, // Hours
        // Individual grade fields
        { name: 'grade1', weight: 0.02 }, 
        { name: 'grade2', weight: 0.02 },
        { name: 'grade3', weight: 0.02 },
        { name: 'grade4', weight: 0.02 },
        { name: 'grade5', weight: 0.02 },
        { name: 'grade6', weight: 0.02 },
        { name: 'grade7', weight: 0.02 },
        { name: 'grade8', weight: 0.02 },
        { name: 'grade9', weight: 0.02 },
        ...
        { name: 'searchableContent', weight: 0.15, getFn: (obj) => {
          ...
          return [
            ...
            obj.notes || '',
            obj.bloodType || '',
            obj.totalHours ? obj.totalHours.toString() : '',
            // Grade information as searchable text
            `پۆلی یەک: ${obj.grade1 || 1}`,
            `پۆلی دوو: ${obj.grade2 || 1}`,
            `پۆلی سێ: ${obj.grade3 || 1}`,
            `پۆلی چوار: ${obj.grade4 || 1}`,
            `پۆلی پێنج: ${obj.grade5 || 1}`,
            `پۆلی شەش: ${obj.grade6 || 1}`,
            `پۆلی حەفت: ${obj.grade7 || 1}`,
            `پۆلی هەشت: ${obj.grade8 || 1}`,
            `پۆلی نۆ: ${obj.grade9 || 1}`,
            // Grade numbers as strings
            [obj.grade1, obj.grade2, obj.grade3, obj.grade4, obj.grade5, 
             obj.grade6, obj.grade7, obj.grade8, obj.grade9].join(' ')
          ].join(' ').toLowerCase()
        }}
      ],
  ...
  const editFields = [
    ...
    },
    {
      key: 'subject',
      label: 'Subject',
      labelKu: 'ناوی وانەکە کە دەیڵێتەوە',
      type: 'text',
    ...
    },
    {
      key: 'totalHours',
      label: 'Total Hours',
      labelKu: 'کۆی بەشە وانە کە دەوترێتەوە',
      type: 'text',
  ...
  const filteredData = useMemo(() => {
    if (!searchTerm.trim()) {
      return Array.isArray(teacherInfoData) ? teacherInfoData : []
    }
    ...
    <PageLayout title="Teacher Information" titleKu="زانیاری مامۆستا">
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-center justify-between">
        <div className="relative flex-1">
          ...
          <Input
            placeholder="گەڕانی فازی لە هەموو ستوونەکانی زانیاری مامۆستایاندا... / Fuzzy search across all teacher info columns..."
            value={searchTerm}
      ...
      <EditModal
        ...
        titleKu="دەستکاریکردنی زانیاری مامۆستا"
      />
