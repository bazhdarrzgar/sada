...
export default function BuildingExpensesPage() {
  ...
  const saveEntry = async (entry) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        resetNewEntry()
        // Refresh the page after successful operation - EXACTLY like kitchen expenses
  ...
  const handleModalSave = async (editedData) => {
    ...
    setEditingData(null)
    // Refresh the page after successful edit operation - EXACTLY like kitchen expenses
    window.location.reload()
  }
            ...
            <div className="space-y-4">
              ...
              <div className="text-center bg-green-50 dark:bg-green-900/20 rounded-lg p-4 border border-green-200 dark:border-green-700">
                <div className="text-lg font-semibold text-green-700 dark:text-green-300 mb-2">
                  {t('buildingExpenses.summary.currentTableTotal', language)} / {t('buildingExpenses.summary.currentTableTotal', 'kurdish')}
                </div>
              ...
              <div className="flex flex-col sm:flex-row gap-4 items-center justify-between border-b border-gray-200 dark:border-gray-600 pb-4">
                <div className="text-lg font-semibold text-gray-700 dark:text-gray-300">
                  {t('buildingExpenses.summary.summaryCalculator', language)} / {t('buildingExpenses.summary.summaryCalculator', 'kurdish')}
                  <div className="text-xs text-gray-500 mt-1">{t('buildingExpenses.summary.filterNote', language)}</div>
                </div>
                  ...
                  {(summaryYear && summaryYear !== "all-years") || (summaryMonth && summaryMonth !== "all-months") ? (
                    <>
                      <p className="text-sm text-gray-600 dark:text-gray-400 font-medium">
                        {(summaryYear && summaryYear !== "all-years") && (summaryMonth && summaryMonth !== "all-months") 
                          ? `${t('buildingExpenses.summary.summaryFor', language)} ${getMonthName(summaryMonth)?.split(' ')[1] || summaryMonth} ${summaryYear} / ${t('buildingExpenses.summary.summaryFor', 'kurdish')} ${getMonthName(summaryMonth)?.split(' ')[1] || summaryMonth} ${summaryYear}`
                          : (summaryYear && summaryYear !== "all-years")
                          ? `${t('buildingExpenses.summary.summaryFor', language)} Year ${summaryYear} / ${t('buildingExpenses.summary.summaryFor', 'kurdish')} ساڵی ${summaryYear}`
                          : `${t('buildingExpenses.summary.summaryFor', language)} Month ${summaryMonth} / ${t('buildingExpenses.summary.summaryFor', 'kurdish')} مانگی ${getMonthName(summaryMonth)?.split(' ')[1] || summaryMonth}`
                        }
                      </p>
                    ...
                    <>
                      <p className="text-sm text-gray-600 dark:text-gray-400 font-medium">{t('buildingExpenses.summary.allRecordsSummary', language)} / {t('buildingExpenses.summary.allRecordsSummary', 'kurdish')}</p>
        ...
        <div className="flex flex-col sm:flex-row gap-4 items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <DialogContent className="max-w-md">
              <DialogHeader>
                <DialogTitle>{t('buildingExpenses.addTitle', language)}</DialogTitle>
              </DialogHeader>
              <div className="space-y-4">
                <div>
                  <Label htmlFor="item">{t('buildingExpenses.fields.item', language)}</Label>
                  <Input
                    id="item"
                    value={newEntry.item}
                ...
                <div>
                  <Label htmlFor="cost">{t('buildingExpenses.fields.cost', language)}</Label>
                  <Input
                    id="cost"
                    type="number"
                    value={newEntry.cost}
                ...
                <div>
                  <Label htmlFor="year">{t('buildingExpenses.fields.year', language)}</Label>
                  <Input
                    id="year"
                    type="number"
                    value={newEntry.year}
                ...
                <div>
                  <Label htmlFor="month">{t('buildingExpenses.fields.month', language)}</Label>
                  <Input
                    id="month"
                    type="number"
                    min="1"
                ...
                <div>
                  <Label htmlFor="notes">{t('buildingExpenses.fields.notes', language)}</Label>
                  <Input
                    id="notes"
                ...
                <div className="flex justify-end gap-2">
                  ...
                  <Button onClick={() => saveEntry(newEntry)} disabled={isSaving}>
                    {isSaving ? 'Saving...' : t('buildingExpenses.buttons.save', language)}
                  </Button>
                </div>
              </div>
            </DialogContent>
