...
export default function ExpensesPage() {
  ...
  const [newEntry, setNewEntry] = useState({
    ...
    electricity: 0,
    total: 0,
    requirement: '', // New field: پێداویستی
  ...
  const fuse = useMemo(() => {
    const options = {
      keys: [
        ...
        { name: 'requirement', weight: 0.25 }, // New field for requirement
        ...
        { name: 'searchableContent', weight: 0.25, getFn: (obj) => {
          return [
            ...
            obj.month || '',
            obj.notes || '',
            obj.requirement || '', // Include requirement in search
  ...
  const handleCellEdit = async (rowIndex, field, value) => {
    try {
      ...
      const entryToUpdate = { ...updatedData[rowIndex] }
      await saveEntry(entryToUpdate)
      
    } catch (error) {
      console.error('Error updating cell:', error)
      // Refresh the page to revert the change if there's an error
      window.location.reload()
    }
  }
  ...
  const saveRowEdit = (rowIndex) => {
    ...
    saveEntry(entry)
    // Refresh the page after successful inline edit operation
    window.location.reload()
  }

  const cancelEdit = () => {
    setEditingRow(null)
    // Refresh page to discard unsaved changes
    window.location.reload()
  }
  ...
  // FIRST FILTER - Controls only Total Monthly Expenses (Summary)
              ...
              {entry.requirement && (
                <div className="border-t pt-2">
                  <div className="text-sm">
                    <span className="font-semibold">پێداویستی:</span>
            ...
            <div className="space-y-4">
              ...
              <div className="text-center bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4 border border-blue-200 dark:border-blue-700">
                <div className="text-lg font-semibold text-blue-700 dark:text-blue-300 mb-2">
                  Total Monthly Expenses / کۆی گشتی خەرجی مانگانە
                </div>
                  ...
                  {(summaryYear && summaryYear !== "all-years") || (summaryMonth && summaryMonth !== "all-months") ? (
                    <>
                      <p className="text-sm text-gray-600 dark:text-gray-400 font-medium">
                        {(summaryYear && summaryYear !== "all-years") && (summaryMonth && summaryMonth !== "all-months") 
                          ? `Summary for ${getMonthName(summaryMonth).split(' ')[1]} ${summaryYear} / کۆی ${getMonthName(summaryMonth).split(' ')[1]} ${summaryYear}`
                          : (summaryYear && summaryYear !== "all-years")
                          ? `Summary for Year ${summaryYear} / کۆی ساڵی ${summaryYear}`
                          : `Summary for Month ${summaryMonth} / کۆی مانگی ${getMonthName(summaryMonth).split(' ')[1]}`
                        }
                      </p>
          ...
          <div className="flex items-center gap-4">
            ...
            <PrintButton 
              ...
              columns={[
                ...
                { key: 'requirement', header: 'پێداویستی' },
            ...
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
              ...
              <div className="space-y-4">
                ...
                <div>
                  <Label htmlFor="requirement">Requirement / پێداویستی</Label>
                  <Textarea
                    id="requirement"
