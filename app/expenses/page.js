...
export default function MonthlyExpensesPage() {
  ...
  const [activeTab, setActiveTab] = useState('main') // 'main' or 'educational'
  ...
  const handleCellEdit = (rowIndex, field, value) => {
    ...
    if (['books', 'clothes', 'travel', 'transportation'].includes(field)) {
      ...
      // Store educational total separately (we'll add this field to the data model)
      updatedData[rowIndex].educationalTotal = books + clothes + travel + transportation
    }
  ...
  const handleModalFieldChange = (newData, fieldKey, value) => {
    ...
    if (['books', 'clothes', 'travel', 'transportation'].includes(fieldKey)) {
      ...
      newData.educationalTotal = books + clothes + travel + transportation
    }
  ...
  // Define fields for modal editing based on active tab
  const editFields = useMemo(() => {
    if (activeTab === 'educational') {
      return [
        ...
        {
          key: 'educationalTotal',
          label: language === 'kurdish' ? 'کۆی گشتی' : 'Total',
          labelKu: 'کۆی گشتی',
          type: 'number',
          placeholder: '0',
          readOnly: true,
  ...
  // Filter data for display based on content (hide empty rows for specific tabs)
  const mainExpensesData = useMemo(() => {
    return filteredData.filter(item => {
      ...
      const hasRequirement = item.requirement && item.requirement.trim().length > 0
      return hasMainExpenses || hasRequirement
    })
  }, [filteredData])

  const educationalExpensesData = useMemo(() => {
    return filteredData.filter(item => {
      const hasEducationalExpenses = (parseFloat(item.books) || 0) +
        (parseFloat(item.clothes) || 0) +
        (parseFloat(item.travel) || 0) +
        (parseFloat(item.transportation) || 0) > 0
      return hasEducationalExpenses
    })
  }, [filteredData])

  // Prepare data for printing based on active tab
  const printData = useMemo(() => {
    if (activeTab === 'educational') {
      return educationalExpensesData.map(item => ({
        ...item,
  ...
  const printColumns = useMemo(() => {
    if (activeTab === 'educational') {
      return [
        ...
        { key: 'books', header: t('monthlyExpenses.fields.books', 'kurdish'), render: (value) => (parseFloat(value) || 0).toLocaleString() },
        { key: 'clothes', header: t('monthlyExpenses.fields.clothes', 'kurdish'), render: (value) => (parseFloat(value) || 0).toLocaleString() },
        { key: 'travel', header: t('monthlyExpenses.fields.travel', 'kurdish'), render: (value) => (parseFloat(value) || 0).toLocaleString() },
        { key: 'transportation', header: t('monthlyExpenses.fields.transportation', 'kurdish'), render: (value) => (parseFloat(value) || 0).toLocaleString() },
        { key: 'educationalTotal', header: t('monthlyExpenses.fields.total', 'kurdish'), render: (value) => (parseFloat(value) || 0).toLocaleString() },
        { key: 'notes', header: t('monthlyExpenses.fields.notes', 'kurdish') }
      ]
    }
    // Main expenses columns
    return [
      { key: 'year', header: t('monthlyExpenses.fields.year', 'kurdish') },
      { key: 'month', header: t('monthlyExpenses.fields.month', 'kurdish') },
      ...
      { key: 'total', header: t('monthlyExpenses.fields.total', 'kurdish'), render: (value) => (parseFloat(value) || 0).toLocaleString() },
      { key: 'receiptImages', header: t('monthlyExpenses.fields.receiptImages', 'kurdish') },
      { key: 'notes', header: t('monthlyExpenses.fields.notes', 'kurdish') }
    ]
  }, [activeTab, language])

  // Prepare data for download based on active tab (filtering columns)
  const downloadData = useMemo(() => {
    if (activeTab === 'educational') {
      return educationalExpensesData.map(item => {
        const educationalTotal = (parseFloat(item.books) || 0) + (parseFloat(item.clothes) || 0) + (parseFloat(item.travel) || 0) + (parseFloat(item.transportation) || 0)
        return {
          year: item.year,
          month: item.month,
          books: item.books,
  ...
  function ExpensesCardView({ data, type = 'main' }) {
    return (
        ...
        {data.map((entry, idx) => (
          <Card key={entry.id} className="p-4 theme-card glow-button hover:shadow-xl transition-all duration-300 border-2 hover:border-blue-300 dark:hover:border-blue-500 group">
            <div className="space-y-3">
              ...
              {/* Requirement section - only for main expenses */}
              {type === 'main' && entry.requirement && (
              ...
              {/* Educational/Travel expenses fields */}
              {type === 'educational' && (
                <div className="grid grid-cols-2 gap-2 text-sm">
                  <div><span className="font-semibold">{t('monthlyExpenses.fields.books', language)}:</span> <span className="text-indigo-600 dark:text-indigo-400">{(parseFloat(entry.books) || 0).toLocaleString()}</span></div>
                  <div><span className="font-semibold">{t('monthlyExpenses.fields.clothes', language)}:</span> <span className="text-pink-600 dark:text-pink-400">{(parseFloat(entry.clothes) || 0).toLocaleString()}</span></div>
                  <div><span className="font-semibold">{t('monthlyExpenses.fields.travel', language)}:</span> <span className="text-teal-600 dark:text-teal-400">{(parseFloat(entry.travel) || 0).toLocaleString()}</span></div>
                  <div><span className="font-semibold">{t('monthlyExpenses.fields.transportation', language)}:</span> <span className="text-cyan-600 dark:text-cyan-400">{(parseFloat(entry.transportation) || 0).toLocaleString()}</span></div>
                </div>
              )}

              {/* Receipt Images - only for main expenses */}
              {type === 'main' && entry.receiptImages && entry.receiptImages.length > 0 && (
                <div className="mt-3">
  ...
  function ExpensesTableView({ data, type = 'main' }) {
    ...
    const educationalColumns = [
      ...
      {
        key: 'educationalTotal',
        header: language === 'kurdish' ? 'کۆی گشتی' : 'Total',
        align: 'center',
        editable: false,
        render: (value, row) => {
          // Calculate educational total from the 4 fields
          const books = parseFloat(row.books) || 0
          const clothes = parseFloat(row.clothes) || 0
          const travel = parseFloat(row.travel) || 0
          const transportation = parseFloat(row.transportation) || 0
          const total = books + clothes + travel + transportation
          return <span className="font-bold text-lg text-blue-600 dark:text-blue-400">{total.toLocaleString()}</span>
        }
      },
      {
    ...
    const columns = type === 'educational' ? educationalColumns : mainColumns
  ...
  return (
    <PageLayout title={t('monthlyExpenses.title', language)} titleKu={t('monthlyExpenses.title', 'kurdish')}>
      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-col sm:flex-row gap-4 items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <DownloadButton
              data={downloadData}
              filename={`monthly-expenses-${activeTab}-records`}
              className="bg-green-600 hover:bg-green-700 text-white"
            />
            <PrintButton
              data={printData}
              filename={`monthly-expenses-${activeTab}-records`}
              title={activeTab === 'educational' ? "خەرجی کەسی و پەروەردەی" : "خەرجی گشتی"}
              titleKu={activeTab === 'educational' ? "خەرجی کەسی و پەروەردەی" : "پوختی حساباتی مانگانە"}
              selectedMonth={selectedMonth !== "all-months" ? selectedMonth : null}
              selectedYear={selectedYear !== "all-years" ? selectedYear : null}
              columns={printColumns}
              showTotal={true}
              totalColumn={activeTab === 'educational' ? 'educationalTotal' : 'total'}
                ...
                <Tabs defaultValue="main" className="w-full">
                  <TabsList className="grid w-full grid-cols-2 mb-4">
                    <TabsTrigger value="main">
                      {language === 'kurdish' ? 'خەرجی گشتی' : 'General Expenses'}
                    </TabsTrigger>
                    <TabsTrigger value="educational">
                      {language === 'kurdish' ? 'خەرجی کەسی و پەروەردەی' : 'Personal & Educational'}
                    </TabsTrigger>
                  </TabsList>
                  ...
                  <TabsContent value="educational" className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <Label htmlFor="edu-year">{t('monthlyExpenses.fields.year', language)}</Label>
                        <Input
                          id="edu-year"
                          type="number"
                          value={newEntry.year}
                      ...
                      <div>
                        <Label htmlFor="edu-month">{t('monthlyExpenses.fields.month', language)}</Label>
                        <Input
                          id="edu-month"
                          type="number"
                          min="1"
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) => {
                            ...
                            const total = value + parseFloat(updatedEntry.clothes) + parseFloat(updatedEntry.travel) + parseFloat(updatedEntry.transportation)
                            updatedEntry.educationalTotal = total
                            setNewEntry(updatedEntry)
                          }}
                          placeholder="0"
                        />
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) => {
                            ...
                            const total = parseFloat(updatedEntry.books) + value + parseFloat(updatedEntry.travel) + parseFloat(updatedEntry.transportation)
                            updatedEntry.educationalTotal = total
                            setNewEntry(updatedEntry)
                          }}
                          placeholder="0"
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) => {
                            ...
                            const total = parseFloat(updatedEntry.books) + parseFloat(updatedEntry.clothes) + value + parseFloat(updatedEntry.transportation)
                            updatedEntry.educationalTotal = total
                            setNewEntry(updatedEntry)
                          }}
                          placeholder="0"
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) => {
                            ...
                            const total = parseFloat(updatedEntry.books) + parseFloat(updatedEntry.clothes) + parseFloat(updatedEntry.travel) + value
                            updatedEntry.educationalTotal = total
                            setNewEntry(updatedEntry)
                          }}
                          placeholder="0"
                    ...
                    <div>
                      <Label htmlFor="educationalTotal">{language === 'kurdish' ? 'کۆی گشتی' : 'Total'}</Label>
                      <Input
                        id="educationalTotal"
                        type="number"
                        value={newEntry.educationalTotal || 0}
                        readOnly
                        className="bg-gray-100"
                      />
                    </div>
                    ...
                    <div>
                      <Label htmlFor="edu-notes">{t('monthlyExpenses.fields.notes', language)}</Label>
                      <Textarea
                        id="edu-notes"
                        value={newEntry.notes}
                        onChange={(e) => setNewEntry({ ...newEntry, notes: e.target.value })}
      ...
      {/* Expenses Table/Cards with Tabs */}
      <div className="mt-6">
        <Tabs defaultValue="main" className="w-full" onValueChange={setActiveTab}>
          <TabsList className="grid w-full grid-cols-2">
            <TabsTrigger value="main">
              {language === 'kurdish' ? 'خەرجی گشتی' : 'General Expenses / خەرجی گشتی'}
            </TabsTrigger>
            <TabsTrigger value="educational">
              {language === 'kurdish' ? 'خەرجی کەسی و پەروەردەی' : 'Personal & Educational / خەرجی کەسی و پەروەردەی'}
            </TabsTrigger>
          </TabsList>

          <TabsContent value="main" className="mt-6">
            {isMobile ? (
              <ExpensesCardView data={mainExpensesData} type="main" />
            ) : (
              <ExpensesTableView data={mainExpensesData} type="main" />
            )}
          </TabsContent>

          <TabsContent value="educational" className="mt-6">
            {isMobile ? (
              <ExpensesCardView data={educationalExpensesData} type="educational" />
            ) : (
              <ExpensesTableView data={educationalExpensesData} type="educational" />
            )}
          </TabsContent>
        </Tabs>
      </div>
