  ...
  return (
    <PageLayout title="Bus Management" titleKu="بەڕێوەبردنی پاس">
      ...
      <Dialog open={isDetailModalOpen} onOpenChange={setIsDetailModalOpen}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto" aria-describedby="bus-detail-description">
          <DialogHeader>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="text-right">
                    ...
                    <div className="mt-1 p-2 bg-white dark:bg-gray-800 rounded border">
                      {selectedRecord.driverLicenseImage ? (
                        typeof selectedRecord.driverLicenseImage === 'object' && selectedRecord.driverLicenseImage.url ? (
                          <img
                            src={selectedRecord.driverLicenseImage.url}
                            alt="Driver License"
                            className="w-32 h-20 rounded object-cover cursor-pointer hover:scale-105 transition-transform"
                            onClick={() => setPreviewImage({ 
                              url: selectedRecord.driverLicenseImage.url, 
                              originalName: selectedRecord.driverLicenseImage.originalName || 'Driver License Image' 
                            })}
                          />
                        ) : selectedRecord.driverLicenseImage.startsWith && selectedRecord.driverLicenseImage.startsWith('http') ? (
                          <img
                            src={selectedRecord.driverLicenseImage}
                            alt="Driver License"
                            className="w-32 h-20 rounded object-cover cursor-pointer hover:scale-105 transition-transform"
                            onClick={() => setPreviewImage({ 
                              url: selectedRecord.driverLicenseImage, 
                              originalName: 'Driver License Image' 
                            })}
                          />
                        ) : (
                          <span className="text-green-600">وێنەی سەنەوی هەیە</span>
                        )
                      ) : (
                        <span className="text-gray-500">وێنەی سەنەوی نییە</span>
                      )}
                    </div>
                  </div>
                </div>
